...
for arg in vars(args):
    print('[%s] =' % arg, getattr(args, arg), flush=True)
...
if not os.path.exists(args.hed_mat_dir):
    print('create output directory %s' % args.hed_mat_dir, flush=True)
    os.makedirs(args.hed_mat_dir)

imgList = os.listdir(args.images_dir)
nImgs = len(imgList)
print('#images = %d' % nImgs, flush=True)
...
for i in range(nImgs):
    if i % 500 == 0:
        print('processing image %d/%d' % (i, nImgs), flush=True)
